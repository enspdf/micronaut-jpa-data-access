package example.micronaut.data.access.repository.impl;

import example.micronaut.data.access.ApplicationConfiguration;
...
public class GenreRepositoryImpl implements GenreRepository {
    ...
    public List<Genre> findAll(SortingAndOrderArguments args) {
        ...
        if (args.getOrder().isPresent() && args.getSort().isPresent() && VALID_PROPERTY_NAMES.contains(args.getSort().get())) {
            qlString += " ORDER BY g." + args.getSort().get() + " " + args.getSort().get().toLowerCase();
        }
